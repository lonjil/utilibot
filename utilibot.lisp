...
(defun parse-dice (string)
  (when (or (= 0 (length string))
            (every 'whitespace-p string))
    (return-from parse-dice (list (cons :modifier nil)
                                  (cons :count 1)
                                  (cons :faces 6))))
...
(define-bot-command "delete" ()
  (private (let* ((split (split-sequence #\space args))
                  (1st (car split))
                  (id (parse-integer 1st :junk-allowed t)))
             (if id
                 (b:delete-message channel-id id)
                 (reply "Invalid ID supplied.")))))
  ...
  (with-accessors ((id b:id) (author b:author) (chid b:channel-id)
    ...
    (let ((user-id (b:id author))
      ...
      (when (and (equal chid (ubi:value 'consent-channel))
        ...
        (b:delete-message chid id)
        (b:send-message chid "Welcome!"))
      (when (equal prefix "&")
        (unless (b:bot? author)
          (multiple-value-bind
                (cmd index)
